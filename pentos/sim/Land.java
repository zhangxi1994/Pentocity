...
public class Land {
	...
	public Land(int side) {
		...
		for (int z = 1; z < side + 2; z++) { // turn 50x50 board into 52x52
												// board with borders included
	...
	public boolean isPond(Cell q) {
		return isPond(q.i, q.j);
	}

	public boolean isField(Cell q) {
		return isField(q.i, q.j);
	}

	public boolean isPond(int i, int j) {
		return isGroup(i, j, Cell.Type.WATER);
	}

	public boolean isField(int i, int j) {
		return isGroup(i, j, Cell.Type.PARK);
	}

	private boolean isGroup(int i, int j, Cell.Type t) {
		if (!land[i][j].isType(t)) {
			return false;
		}
	...
	public boolean unoccupied(Cell q) {
		return unoccupied(q.i, q.j);
	}

	public boolean unoccupied(int i, int j) {
		return i >= 0 && i < land.length && j >= 0 && j < land[i].length && land[i][j].isEmpty();
	}
	...
	public boolean buildable(Building building, Cell q) {
		for (Cell p : building) {
			...
			for (Cell x : adj) {
				if ((land[x.i][x.j].type == Cell.Type.FACTORY && building.type == Building.Type.RESIDENCE)
						|| (land[x.i][x.j].type == Cell.Type.RESIDENCE && building.type == Building.Type.FACTORY))
					return false;
			}
		}
		return true;
	...
	protected void buildRoad(Cell q) {
		road_network.add(new Cell(q.i + 1, q.j + 1)); // re-index to allow
														// borders
	...
	// return -1 if building cannot be built. Otherwise return the score accrued
	...
	protected int build(Building building, Cell q) {
		...
		for (Cell p : building) {
			Cell[] adj = land[p.i + q.i][p.j + q.j].neighbors();
			for (Cell a : adj) {
				adjacent_points.add(a); // also includes building cells but
